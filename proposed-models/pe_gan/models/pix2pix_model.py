...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # only defined during training time
            ...
            self.weight = torch.nn.Parameter(torch.tensor(0.01), requires_grad=True)

            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG1.parameters(), self.netG2.parameters(), [self.weight]), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def backward_G(self):
        ...
        self.loss_G_correction = (self.loss_G_correction + self.loss_G_correction1) * self.weight
        self.loss_G_coherence = (self.loss_G_coherence + self.loss_G_coherence1) * self.weight
