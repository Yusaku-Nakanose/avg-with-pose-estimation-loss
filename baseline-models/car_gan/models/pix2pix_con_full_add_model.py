...
class Pix2PixconfulladdModel(BaseModel):
    ...
    def forward(self):
        ...
        #fake_all = torch.chunk(f, 4, dim=1)
        """ self.fake_image1_1 = fake_all[0]
        self.fake_image1_2 = fake_all[1]
        self.fake_image1_3 = fake_all[2]
        self.fake_image1_4 = fake_all[3] """
        #ここの入力画像を変更する
        #print(self.real_audio.size())
        #mask = torch.ones(128, 128).to(self.device)
        """ mask1 = torch.ones(128, 128)
        mask2 = torch.ones(128, 128)
        mask3 = torch.ones(128, 128)
        mask4 = torch.ones(128, 128) """
        mask1 = torch.zeros(4, 1, 128, 128)
        mask2 = torch.zeros(4, 1,128, 128)
        mask3 = torch.zeros(4, 1,128, 128)
        mask4 = torch.zeros(4, 1,128, 128)
        mask1[:, :, :,12:39] = 1 #13～39
        mask2[:, :, :,38:64] = 1
        mask3[:, :, :,63:89] = 1
        mask4[:, :, :,88:114] = 1
        ...
        #print(torch.cat([self.real_audio, mask1], dim=1).size())
        #print(a)
        ...
        #print(self.fake_image1_1.size())
        
        #netG1とG2の大きさをそろえるためにゼロうめした層を追加
        #print(self.cat_input(torch.cat([self.fake_image1_1, torch.zeros(4, 1, 128, 128).to(self.device)],dim=1), 2).size())
        self.fake_image2_1 = self.netG2(self.cat_input(torch.cat([self.fake_image1_1, torch.zeros(4, 1, 128, 128).to(self.device)],dim=1), 2))
        self.fake_image2_2 = self.netG2(self.cat_input(torch.cat([self.fake_image1_2, torch.zeros(4, 1, 128, 128).to(self.device)],dim=1), 2))
        self.fake_image2_3 = self.netG2(self.cat_input(torch.cat([self.fake_image1_3, torch.zeros(4, 1, 128, 128).to(self.device)],dim=1), 2))
        self.fake_image2_4 = self.netG2(self.cat_input(torch.cat([self.fake_image1_4, torch.zeros(4, 1, 128, 128).to(self.device)],dim=1), 2))
